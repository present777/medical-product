...
def generate_oldcart_case():
    ...
    【强制任务】
    你现在是医学考核出题人。请务必根据以下指令生成一个标准化病人（SP）。
    ...
    **核心特征参考**: {target['hint']}
    ...
    3. **OLD CART 完整**：必须设计出完整的现病史：
       - Onset (起病，如缓慢/急性)
       - Location (部位)
       - Duration (病程)
       - Characteristics (性质，如压榨样/烧灼样)
       - Aggravating/Relieving (加重/缓解因素)
       - Treatment (诊疗经过)
...
def run_evaluation(chat_history, user_diagnosis,correct_disease):
    ...
    # 4. 评估 Prompt
    evaluator_prompt = f"""
    你现在是【临床医学考核系统】。请生成一份含“正确答案”和“精准引用”的评估报告。
    ...
    2. **语义等效性识别 (Semantic Equivalence)**：这是最重要的！
    - 考生的对象是普通患者，**必须使用通俗语言**。
    ...
    3. **证据溯源**：虽然语言可以通俗，但必须真的问了。如果连“按肚子”都没问，就不能算查了“压痛”。

    4. **拒绝送分**：如果对话记录里没有体现，无论标准病例里有没有，一律判错。

    5. **必须揭晓答案**：在表格第一行明确展示“标准诊断”。
    ...
    7. **自动打分**：请严格按照以下标准扣分。
       - **诊断结论 (30分)**：诊断完全正确得30分，错误或缺失得0分。
       - **信息采集 (50分)**：核心指征每漏一个扣 10 分，扣完为止。
       - **问诊规范 (20分)**：是否有诱导提问、是否体现关怀。
...
def main():
    ...
    if "status" not in st.session_state:
        ...
        【表演规则】
        1. **被动触发**：只有医生问到了对应的 OLD CART 维度，你才回答相应信息。
       - 例如：如果不问 "怎么痛"，你就只说 "痛"，不要主动说 "绞痛"。
        2. **通俗化**：不要说 "上腹部压痛"，要说 "肚子上面这块，按着疼"。
    ...
    if st.session_state.status == "chatting":
        ...
        # 侧边栏：交卷按钮
        with st.sidebar:
            st.markdown("💡 **提示**：请尝试按照 OLD CART 原则提问。")
            st.divider()
